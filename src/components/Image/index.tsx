...
const Image: React.FC<ImageProps> = ({
	...
	const imageSourceWithoutProtocol = url.replace("https://", "").replace("http://", "");
	...
	if (
		typeof resizedImageOptions === "undefined" ||
		typeof resizedImageOptions[`${largeWidth}x${largeHeight}`] === "undefined"
	) {
		...
		<StyledPicture key={url}>
			...
			{typeof lightBoxWidth === "undefined" || typeof lightBoxHeight === "undefined" ? (
				<img
					alt={alt}
					src={buildThumborURL(
						resizedImageOptions[`${largeWidth}x${largeHeight}`],
				...
				<img
					...
					data-lightbox={buildThumborURL(
						resizedImageOptions[`${lightBoxWidth}x${lightBoxHeight}`],
						`${lightBoxWidth}x${lightBoxHeight}`,
						imageSourceWithoutProtocol,
						resizerURL
					)}
					width={largeWidth}
...
Image.propTypes = {
	...
	loading: PropTypes.oneOf<LoadingTypes>([LoadingTypes.Eager, LoadingTypes.Lazy]),
};
