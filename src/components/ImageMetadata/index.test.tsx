...
describe("the ImageMetadata component", () => {
	it("should render a styled p with the color correctly passed in", () => {
		const wrapper = shallow(<ImageMetadata subtitle="ffff" />);
	...
	describe("when no credits are passed in the props", () => {
		it("should not include the credits string", () => {
			const wrapper = mount(<ImageMetadata subtitle="ffffg" caption="ttttu" />);
	...
	describe("when no affiliation is passed in the props", () => {
		it("should not include the affliation in the credits string", () => {
			const wrapper = mount(
				<ImageMetadata subtitle="ffffg" caption="ttttu" credits={{ by: [{ name: "bbbyyy" }] }} />
			);
			expect(wrapper.find("span")).toHaveLength(2);
			expect(wrapper.find("span").first().text()).toBe("ffffg ");
		...
		it("should override photographer and credit on image using vanity credits info", () => {
			const wrapper = mount(
				<ImageMetadata
					...
					credits={{ by: [{ name: "bbbyyy" }], affiliation: [{ name: "affff" }] }}
					vanityCredits={{
						by: [{ name: "vanity photograher" }],
						affiliation: [{ name: "vanity credit" }],
					}}
				/>
			);
			expect(wrapper.find("span")).toHaveLength(2);
		...
		it("should concatenate the creators using a comma", () => {
			const wrapper = mount(
				<ImageMetadata
					...
					caption="ttttu"
					credits={{
						by: [{ name: "bbbyyy" }, { name: "bbbyyy2" }],
						affiliation: [{ name: "affff" }],
					}}
				/>
			);
			expect(wrapper.find("span")).toHaveLength(2);
		...
		it("should concatenate the affilites using a comma", () => {
			const wrapper = mount(
				<ImageMetadata
					...
					credits={{
						by: [{ name: "bbbyyy" }],
						affiliation: [{ name: "affff" }, { name: "affff2" }],
					}}
				/>
			);
			expect(wrapper.find("span")).toHaveLength(2);
